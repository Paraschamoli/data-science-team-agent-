...
def create_histogram(plot_dict: dict[str, Any]) -> go.Figure:
    ...
    fig = px.histogram(
        df, x=x_col, color=color_col, title=plot_dict.get("title", "Histogram"), labels=plot_dict.get("labels", {})
    )
...
def create_heatmap(plot_dict: dict[str, Any]) -> go.Figure:
    ...
    fig = px.imshow(df, title=plot_dict.get("title", "Heatmap"), labels=plot_dict.get("labels", {}))
